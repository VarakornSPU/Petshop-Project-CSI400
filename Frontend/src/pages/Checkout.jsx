...
export default function Checkout() {
    ...
    function getProductImage(item) {
        // 1. ถ้ามี imageUrl (มาจาก CartContext)
        if (item.imageUrl) return item.imageUrl;

        // 2. ถ้ามี image (string)
        if (typeof item.image === "string") {
            if (item.image.startsWith("http")) return item.image;
            if (item.image.startsWith("/uploads")) return `http://localhost:3001${item.image}`;
            return `http://localhost:3001/uploads/${item.image}`;
        }

        // 3. ถ้ามี images (array หรือ string JSON)
        if (item.images) {
            try {
                const imgs = Array.isArray(item.images)
                    ? item.images
                    : JSON.parse(item.images.replace(/{|}/g, "[").replace(/,/g, ","));
                if (imgs.length > 0) {
                    const img = imgs[0];
