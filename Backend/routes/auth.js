...
const registerValidation = [
  ...
  body("address.subdistrict")
    .trim()
    .notEmpty()
    .withMessage("กรุณากรอกตำบล/แขวง"),
...
router.post("/register", authLimiter, registerValidation, async (req, res) => {
  ...
  try {
    ...
    await client.query(
      ...
      [newUser.id, refreshTokenHash, refreshExpiry]
    );
  ...
  try {
    ...
    if (user.is_deleted) {
      await logAudit({
        userId: user.id,
        action: "login_failed",
        details: { reason: "deleted_account" },
        req,
      });
...
router.post("/refresh", async (req, res) => {
  try {
    ...
    const tokenResult = await pool.query(
      `SELECT rt.id AS rt_id, rt.user_id, rt.expires_at, rt.revoked, u.email, u.role, u.is_deleted, u.is_banned
    ...
    // Rotation: issue a new refresh token and revoke the old one
    ...
    // Start a transaction to rotate tokens atomically
    const client = await pool.connect();
    try {
      await client.query("BEGIN");

      // Mark the current token revoked and set replaced_by after inserting new token
      const insertResult = await client.query(
        "INSERT INTO refresh_tokens (user_id, token_hash, expires_at) VALUES ($1, $2, $3) RETURNING id",
        [token.user_id, newRefreshTokenHash, newRefreshExpiry]
      );
  ...
  async (req, res) => {
    try {
      ...
      if (!emailResult.success) {
        ...
        return res
          .status(500)
          .json({ error: "ไม่สามารถส่งอีเมลรีเซ็ตรหัสผ่านได้" });
      }
  ...
  async (req, res) => {
    try {
      ...
      if (result.rows.length === 0) {
        return res
          .status(400)
          .json({ error: "โทเค็นรีเซ็ตไม่ถูกต้องหรือหมดอายุแล้ว" });
      }
  ...
  async (req, res) => {
    try {
      ...
      if (!errors.isEmpty()) {
        return res
          .status(400)
          .json({ error: "ข้อมูลไม่ถูกต้อง", details: errors.array() });
      }
      ...
      if (phone) {
        ...
        if (existingPhone.rows.length > 0) {
          return res
            .status(400)
            .json({ error: "เบอร์โทรศัพท์นี้ถูกใช้งานแล้ว" });
        }
      }
